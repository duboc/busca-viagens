...
export default function FlightDetailModal({ flight, open, onClose }: FlightDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl max-w-lg w-full shadow-xl dark:shadow-2xl dark:shadow-black/30 max-h-[90vh] overflow-y-auto transition-colors">
        <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 px-6 py-4 flex items-center justify-between rounded-t-2xl">
          <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">Detalhes do Voo</h2>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 space-y-6">
          ...
          <div className="text-center">
            ...
            {flight.badges?.map((b) => (
              <span
                key={b}
                className="inline-block mt-1 mx-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300"
              >
                {b}
              </span>
            ))}
          ...
          <Section title="Ida">
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-gray-500 dark:text-gray-400">{formatDate(flight.outboundDeparture)}</span>
              </div>
              <FlightTimeline
            ...
            <Section title="Volta">
              <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400">{formatDate(flight.returnDeparture)}</span>
                </div>
                <FlightTimeline
          ...
          {flight.rankReasoning && (
            <Section title="Analise">
              <p className="text-sm text-gray-600 dark:text-gray-400 italic">&ldquo;{flight.rankReasoning}&rdquo;</p>
            </Section>
          )}
...
function DetailItem({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="font-medium text-gray-700 dark:text-gray-300 capitalize">{value}</p>
    </div>
  );
}
